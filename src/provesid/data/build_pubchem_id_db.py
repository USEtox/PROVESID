...
Usage:
    python build_pubchem_id_db.py

Requirements:
    - PubChem_CAS_202601.csv file in the same directory
...
if __name__ == '__main__':
    ...
    # Paths
    csv_path = os.path.join(script_dir, 'PubChem_CAS_202601.csv')
    db_path = os.path.join(script_dir, 'pubchem_id.db')
    
    if not os.path.exists(csv_path):
        print(f"Error: CSV file not found at {csv_path}")
        print(f"\nPlease ensure PubChem_CAS_202601.csv is in the same directory as this script:")
        print(f"  {script_dir}")
        sys.exit(1)
