...
class PubChemView:
    ...
    def get_property_table(self, cid: Union[int, str], property_name: str) -> pd.DataFrame:
        ...
        try:
            ...
            for data in property_data:
                ...
                # Parse experimental value and unit from the StringWithMarkup
                exp_value, unit = self._extract_experimental_value_and_unit(data.value)
                
                table_data.append({
                    "CID": cid,
                    "StringWithMarkup": data.value,
                    "ExperimentalValue": exp_value,
                    "Unit": unit if unit else data.unit,  # Use parsed unit or fallback
            ...
            return pd.DataFrame(columns=["CID", "StringWithMarkup", "ExperimentalValue", "Unit", "FullReference"])
    ...
    def _extract_experimental_value_and_unit(self, value_str: str) -> tuple[Optional[str], Optional[str]]:
        """
        Extract numerical experimental value and unit from StringWithMarkup
        
        Args:
            value_str: Full StringWithMarkup value
            
        Returns:
            Tuple of (experimental_value, unit)
        """
        import re
        ...
        # Priority-ordered patterns for extracting the main experimental value
        # (not conditions like "at 20°C")
        
        # 1. Viscosity patterns (high priority for viscosity values)
        viscosity_match = re.search(r'^(\d+(?:\.\d+)?)\s*(cP|mPa·s|Pa·s|cSt)\b', value_str)
        if viscosity_match:
            return viscosity_match.group(1), viscosity_match.group(2)
        
        # 2. Range patterns at start (e.g., "138-140", "135-140 °C")
        range_match = re.search(r'^(-?\d+(?:\.\d+)?-\d+(?:\.\d+)?)\s*([°]?[CF]|K)?\b', value_str)
        if range_match:
            return range_match.group(1), range_match.group(2)
        
        # 3. Temperature patterns at start (main value, not conditions)
        temp_match = re.search(r'^(-?\d+(?:\.\d+)?)\s*[°]?([CF]|K)\b', value_str)
        if temp_match:
            return temp_match.group(1), temp_match.group(2)
        
        # 4. Density patterns
        density_match = re.search(r'^(-?\d+(?:\.\d+)?)\s*(g/cm³|g/mL|kg/m³|g/L)\b', value_str)
        if density_match:
            return density_match.group(1), density_match.group(2)
        
        # 5. Pressure patterns (not conditions)
        pressure_match = re.search(r'^(-?\d+(?:\.\d+)?)\s*(mmHg|kPa|Pa|atm|bar|torr)\b', value_str)
        if pressure_match:
            return pressure_match.group(1), pressure_match.group(2)
        ...
        if sci_match:
            return sci_match.group(1), sci_match.group(2)
